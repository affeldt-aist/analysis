...
Definition is_itvoo (R : realType) (A : set R) := exists ab, A = `]ab.1, ab.2[%classic.

Definition open_itv_cover {R : realType} (X : set R) :=
  [set F : (set R)^nat | (forall i, is_itvoo (F i)) /\ X `<=` \bigcup_k (F k)].

(* wip *)
Module li.

Lemma cvg_geometric_eseries_half {R : archiFieldType} (r : R) (n : nat) :
   eseries (fun k => (r / (2 ^ (k + n.+1))%:R)%:E) x @[x --> \oo] --> (r / 2 ^+ n)%:E.
Proof.
apply: cvg_EFin => //.
  apply: nearW => //= x.
  by rewrite /eseries/= sumEFin.
rewrite [X in X @ _ --> _](_ : _ = (series (fun k : nat => (r / (2 ^ (k + n.+1))%:R)))); last first.
...
Lemma outer_open_itv_cover (X : set R) : (l^*)%mu X =
  ereal_inf [set \sum_(k <oo) l (A k) | A in open_itv_cover X]%E.
Proof.
apply/eqP; rewrite eq_le; apply/andP; split.
  apply: le_ereal_inf => x /= [s].
  rewrite /open_itv_cover/= => -[si Xs <-{x}].
  have Hs i : is_itvoo (s i) := si i.
  exists (fun i => `](sval (cid (Hs i))).1, (sval (cid (Hs i))).2]%classic).
    split.
      move=> i.
      have [H|H] := ltP (sval (cid (Hs i))).1 (sval (cid (Hs i))).2.
        apply/ocitvP; right.
        eexists.
          exact: H.
        by [].
      apply/ocitvP; left.
      by rewrite set_itv_ge// -leNgt.
    move/subset_trans : Xs; apply.
    move=> A/= [n _ snA].
    exists n => //.
    have := Hs n.
    move: snA.
    case: cid => -[x y]/= ->/= Axy _.
    exact: subset_itv_oo_oc Axy.
  apply: eq_eseriesr => k _.
  rewrite /l wlength_itv/=.
  move: (Hs k).
  case => /= -[a b]/= skab.
  case: cid => /= -[x1 x2] ->.
  by rewrite wlength_itv.
have [|] := boolP ((l^* X)%mu \is a fin_num); last first.
  rewrite ge0_fin_numE ?outer_measure_ge0// -leNgt leye_eq => /eqP ->.
  exact: leey.
move/lb_ereal_inf_adherent => H.
apply/lee_addgt0Pr => /= e e0.
have e20 : (0 < e / 2)%R by rewrite divr_gt0//.
have [_ [/= J_ [mJ_ XJ_]] <- lJ_e] := H _ e20.
rewrite -/((l^* X)%mu) in lJ_e.
have I_proof n : exists2 J, J_ n `<=` J &
  (is_itvoo J) /\ (l J <= l (J_ n) + (e / 2 ^+ n.+2)%:E)%E.
  have [[a b] _ /= abJ] := mJ_ n.
  ...
  split.
    by exists (a, b + e / 2^+n.+2)%R.
  have [ab|ba] := ltP a b.
    rewrite /l -abJ !wlength_itv//= !lte_fin ifT; last by rewrite ltr_wpDr// divr_ge0// ltW.
    by rewrite ab -!EFinD lee_fin addrAC.
  rewrite -abJ [in leRHS]set_itv_ge ?bnd_simp -?leNgt// /l wlength0 add0r wlength_itv//=.
  case: ifPn => [|_]; last first.
    by rewrite lee_fin divr_ge0// ltW.
  move=> abe.
  by rewrite -EFinD addrAC lee_fin -[leRHS]add0r lerD2r subr_le0.
pose I_ := fun n => sval (cid2 (I_proof n)).
have H1 n : J_ n `<=` I_ n by rewrite /I_; case: cid2.
have H2 n : is_itvoo (I_ n) by rewrite /I_; case: cid2 => ? ? [].
have H3 n : (l (I_ n) <= l (J_ n) + (e / 2 ^+ n.+2)%:E)%E by rewrite /I_; case: cid2 => ? ? [].
have XI_ : X `<=` \bigcup_k I_ k.
  by move=> z Xz; have [n _ J_nz] := XJ_ _ Xz; exists n => //; exact: H1.
apply: (@le_trans _ _ (\sum_(0 <= k <oo) (l (J_ k) + (e / 2 ^+ k.+2)%:E))%E).
  apply: (@le_trans _ _ (\sum_(0 <= k <oo) (l (I_ k)))%E).
    by apply: ereal_inf_lb => /=; exists I_.
  exact: lee_nneseries.
rewrite nneseriesD//; last first.
  by move=> i _; rewrite lee_fin// divr_ge0// ltW.
rewrite [in leRHS](splitr e) EFinD addeA leeD//.
  exact/ltW.
have := @cvg_geometric_eseries_half R (e) 1.
rewrite expr1.
rewrite (_ : (fun k : nat => (e / (2 ^ (k + 2))%:R)%:E) =
             (fun k : nat => (e / (2 ^+ (k.+2))%:R)%:E)); last first.
  by apply/funext => n; rewrite addn2 natrX.
move/cvg_lim => <-//.
apply: lee_nneseries => //.
- by move=> ? _; rewrite lee_fin divr_ge0// ltW.
...
Lemma outer_open_approx (X : set R) : (l^* X)%mu < +oo ->
  forall e, (0 < e)%R -> exists U, [/\ open U,
    X `<=` U & mu U <= (l^* X)%mu + e%:E].
Proof.
move=> Xoo/= e e0.
have : exists2 I_, open_itv_cover X I_ &
    \sum_(0 <= k <oo) l (I_ k) <= ((wlength idfun)^*)%mu X + e%:E.
  have : (l^* X)%mu\is a fin_num by rewrite ge0_fin_numE// outer_measure_ge0.
  rewrite outer_open_itv_cover.
  move=> /lb_ereal_inf_adherent => /(_ _ e0)[x/= [I]] XI <-{x} Ie.
  by exists I => //; exact/ltW.
move=> [I_ XI_ sumI_].
...
- apply: bigcup_open => // i.
  ...
  exact: interval_open.
- by case: XI_.
- rewrite (le_trans _ sumI_)//.
  have := outer_measure_sigma_subadditive mu I_.
  move=> /le_trans; apply.
...
Lemma outer_Gdelta (X : set R) : (l^* X)%mu < +oo ->
  exists G_ : (set R)^nat, [/\ (forall i, open (G_ i)),
    X `<=` \bigcap_i G_ i &
    mu (\bigcap_i G_ i) = ((wlength idfun)^*)%mu X].
Proof.
move=> Xoo.
have inv0 (k : nat) : (0 < k.+1%:R^-1 :> R)%R by rewrite invr_gt0.
pose U_ (k : nat) := projT1 (cid (outer_open_approx Xoo (inv0 k))).
have oU_ (k : nat) : open (U_ k) by rewrite /U_; case: cid => x /= [].
have XU_ (k : nat) : X `<=` U_ k by rewrite /U_; case: cid => x /= [].
have mUX (k : nat) : mu (U_ k) <= ((wlength idfun)^*)%mu X + k.+1%:R^-1%:E.
...
by apply: le_outer_measure; exact: sub_bigcap.
Unshelve. all: by end_near. Qed.

(* corollary 22 in Li *)
Lemma negligible_outer (N : set R) : mu.-negligible N <-> (l^*)%mu N = 0.
Proof.
split.
  move=> [/= A [mA cmuA NA]].
...
Local Open Scope ereal_scope.
Lemma eseries_mkcond2 [R : realFieldType] [P : pred nat] (f : nat -> \bar R) (N : nat) :
  \sum_(N <= i <oo | P i) f i = \sum_(N <= i <oo) if P i then f i else 0.
Proof. by apply/congr_lim/eq_fun => n /=; apply: big_mkcond. Qed.

Lemma nneseries_tail_cvg_cond (R : realType) (f : (\bar R)^nat) P :
  \sum_(0<= k <oo | P k) f k < +oo -> (forall k, P k -> 0 <= f k) ->
  \sum_(N <= k <oo | P k) f k @[N --> \oo] --> 0.
Proof.
rewrite eseries_mkcond => Pfoo Pf0.
under eq_fun do rewrite eseries_mkcond2.
have := nneseries_tail_cvg Pfoo.
by apply => k; case : ifPn => //; exact: Pf0.
Qed.

Lemma lee_nneseries2 (R : realType) (u : (\bar R)^nat) (P Q : pred nat) (N : nat) :
  (forall i, Q i -> 0 <= u i) ->
  (forall n, P n -> Q n) ->
...
apply: lee_sum_nneg_natr => // n Mn /PQ.
exact: Pu.
apply: ereal_nondecreasing_is_cvgn => a b ab.
apply: lee_sum_nneg_natr => // n Mn.
exact: Pu.
near=> n.
apply: lee_sum_nneg_subset => //.
move=> i; rewrite inE => /andP[iP iQ].
apply: Pu.
done.
Unshelve. all: by end_near. Qed.
...
Proof.
move=> NPf.
rewrite eseries_mkcond2.
rewrite [X in _ = (_ + X)]eseries_mkcond2.
rewrite big_mkcond/=.
...
have muG'fin r : mu (\bigcup_(k in G' r) closure (B k)) \is a fin_num.
  rewrite ge0_fin_numE//.
  rewrite (@le_lt_trans _ _ (mu O))//; last first.
    by move: OAoo => /andP[OA]; exact: lt_trans.
  rewrite (@le_trans _ _ (mu (\bigcup_(k in G) B k)))//.
    rewrite bigcup_mkcond [in leRHS]bigcup_mkcond.
    rewrite measure_bigcup//=; last 2 first.
      move=> i _; case: ifPn => // iG.
      case: ABF' => + _ => /(_ i)/ballE ->.
      rewrite closure_ball.
      exact: measurable_closed_ball.
      apply/(trivIset_mkcond _ _).1.
      apply/trivIsetP => /= i j [/set_mem Gi ri] [/set_mem Gj rj] ij.
      by move/trivIsetP : tB => /(_ _ _ Gi Gj ij).
    rewrite measure_bigcup//=; last 2 first.
      move=> i _; case: ifPn => // iG.
      case: ABF' => + _ => /(_ i)/ballE ->.
      exact: measurable_ball.
      apply/(trivIset_mkcond _ _).1.
      apply/trivIsetP => /= i j  Gi Gj ij.
      move/trivIsetP : tB => /(_ _ _ Gi Gj ij).
      by apply: subsetI_eq0; exact: subset_closure.
    apply: lee_nneseries => // n _.
    case: ifPn => [/set_mem[-> _]|].
    case: ABF' => + _ => /(_ n)/ballE ->.
      rewrite closure_ball.
      rewrite lebesgue_measure_closed_ball//.
      by rewrite lebesgue_measure_ball//.
    by rewrite measure0.
  rewrite le_outer_measure//.
  by apply: bigcup_sub => i /GV'[].
have G'0E : G' 0%R = G.
  by apply/seteqP; split => [x [/set_mem //]|Gx x]; split => //; exact/mem_set.
have muBfin : mu (\bigcup_(k in G) closure(*rm closure*) (B k)) \is a fin_num.
  by rewrite -G'0E.
have G'c (c : {posnum R}) : finite_set (G' c%:num).
  apply: contrapT.
  pose M := `|ceil (fine (mu O) / c%:num)|.+1%N.
  move/infinite_set_fset => /= => /(_ M)[G0 G0G'c] MG0.
  have H1 : mu O < mu (\bigcup_(k in G' c%:num) closure (B k)).
    apply: (@lt_le_trans _ _ (mu (\bigcup_(k in [set` G0]) closure (B k)))).
      rewrite bigcup_fset measure_fbigsetU//=; last 2 first.
        move=> k _.
        case: ABF' => + _ => /(_ k)/ballE ->.
        rewrite closure_ball.
        exact: measurable_closed_ball.
        apply: sub_trivIset tB => x.
        by move/G0G'c => -[/set_mem].
      apply: (@lt_le_trans _ _ (\sum_(i <- G0) c%:num%:E)%R).
        rewrite sumEFin.
        rewrite big_const_seq iter_addr addr0 -mulr_natr.
        apply: (@lt_le_trans _ _ (c%:num * M%:R)%:E).
          rewrite EFinM.
          rewrite -lte_pdivrMl//.
          rewrite muleC.
          rewrite -(@fineK _ (mu O)); last first.
            rewrite ge0_fin_numE//.
            case/andP: OAoo => ?.
            by apply: lt_trans.
          rewrite -EFinM.
          rewrite /M.
          rewrite lte_fin.
          rewrite (le_lt_trans (ceil_ge _))//.
          rewrite -natr1.
          rewrite natr_absz.
          rewrite ger0_norm//; last first.
            by rewrite ceil_ge0// divr_ge0// fine_ge0//.
          by rewrite ltrDl//.
      rewrite lee_fin ler_wpM2l// ler_nat.
      by rewrite count_predT.
    rewrite big_seq [in leRHS]big_seq.
    apply: lee_sum => //= i /G0G'c [iG cBi].
    rewrite -[leRHS]fineK//; last first.
      case: ABF' => + _ => /(_ i)/ballE ->.
      rewrite closure_ball.
      by rewrite lebesgue_measure_closed_ball//.
    case: ABF' => + _ => /(_ i)/ballE ->.
    rewrite closure_ball.
    rewrite lebesgue_measure_closed_ball//.
    rewrite fineK//.
    ...
    rewrite closure_ball.
    exact: measurable_closed_ball.
    by apply: subset_bigcup2.
  have H2 : mu (\bigcup_(k in G' c%:num) closure (B k)) <= mu O.
    apply: (@le_trans _ _ (mu (\bigcup_(k in G' c%:num) (B k)))).
      rewrite measure_bigcup//; last 2 first.
        move=> k _.
        case: ABF' => + _ => /(_ k)/ballE ->.
        rewrite closure_ball.
        exact: measurable_closed_ball.
        by apply: sub_trivIset tB => x [/set_mem].
      rewrite /=.
      rewrite measure_bigcup//; last 2 first.
        move=> k _.
        case: ABF' => + _ => /(_ k)/ballE ->.
    ...
    exact: measurable_ball.
    exact: open_measurable.
    by apply: bigcup_sub => i [/set_mem /GV'[? ?] cBi].
    by rewrite ltNge H2 in H1.
have [c Hc] : exists c : {posnum R},
    (mu (\bigcup_(k in G' c%:num) closure (B k)) >
     mu (\bigcup_(k in G) closure (B k)) - e%:E)%E.
  have : \big[+%R/0%R]_(N <= k <oo | mem G k) mu (closure (B k)) @[N --> \oo] --> 0%E.
    have : \big[+%R/0%R]_(0 <= i <oo | i \in G) mu (closure (B i)) < +oo.
      rewrite -measure_bigcup//=; last first.
        move=> i Gi.
        case: ABF' => + _ => /(_ i)/ballE ->.
        rewrite closure_ball.
        exact: measurable_closed_ball.
      by move: muBfin; rewrite ge0_fin_numE//.
    by move/(@nneseries_tail_cvg_cond _ _ (mem G)); apply.
  move/fine_cvgP => [H1].
  move/cvgrPdist_lt => /(_ e).
  rewrite -lte_fin => /(_ e0)[n _ /=] H2.
  have [c nc] : exists c : {posnum R}, forall k, (c%:num > (radius (B k))%:num)%R -> (k >= n)%N.
    (* take the minimum radius for [0, k] *)
    pose x : R := (\big[minr/(radius (B 0))%:num]_(0 <= i < n.+1) (radius (B i))%:num)%R.
    have x_gt0 : (0 < x)%R.
      rewrite /x.
      by apply: lt_bigmin => //.
    exists (PosNum x_gt0) => k /=.
    rewrite /x.
    apply: contraTT.
    rewrite -ltnNge -leNgt => kn.
    rewrite big_mkord.
    apply/bigmin_leP.
    right.
    rewrite /=.
  ...
  exists c.
  suff:  mu (\bigcup_(k in G `\` G' c%:num) closure (B k)) < e%:E.
    rewrite EFinN lteBlDl// -lteBlDr//.
    apply: le_lt_trans.
    have K1 : (\bigcup_(k in G) closure (B k)) `\` (\bigcup_(k in G' c%:num) closure (B k)) `<=`
        \bigcup_(k in G `\` G' c%:num) closure (B k).
      move=> /= x [[k Gk Bkx]].
      rewrite -[X in X -> _]/((~` (\bigcup_(k0 in G' c%:num) closure (B k0))) x).
      rewrite setC_bigcup => abs.
      exists k => //; split=> // G'ck.
      by have := abs k G'ck.
    apply: (@le_trans _ _ (mu (\bigcup_(k in G) closure (B k) `\` \bigcup_(k in G' c%:num) closure (B k)))); last first.
      rewrite le_measure ?inE//.
      apply: measurableD.
      apply: bigcup_measurable => k _.
      case: ABF' => + _ => /(_ k)/ballE ->.
    ...
    rewrite leeB// le_measure ?inE//.
      ...
      rewrite closure_ball.
      exact: measurable_closed_ball.
  apply: (@le_lt_trans _ _ (normr (0 - fine (\big[+%R/0]_(n <= k <oo | k \in G) mu (closure (B k)))))%:E).
    ...
    rewrite fineK//; last first.
      move: muBfin.
      rewrite measure_bigcup//=; last 2 first.
        rewrite ge0_fin_numE//; last by exact: nneseries_ge0.
        rewrite ge0_fin_numE//; last by exact: nneseries_ge0.
        apply: le_lt_trans.
        rewrite [leRHS](nneseries_split_cond n)//.
        rewrite add0n.
        rewrite leeDr//.
        by apply: sume_ge0 => //.
      move=> i Gi.
    ...
    rewrite measure_bigcup//=; last 2 first.
      ...
      rewrite closure_ball.
      exact: measurable_closed_ball.
      apply: sub_trivIset tB.
      exact: subDsetl.
    rewrite [in leRHS]eseries_cond.
    suff: forall i, i \in G `\` G' c%:num -> (n <= i)%N.
      move=> GG'n.
      apply: lee_nneseries2 => // m mGG'.
      rewrite GG'n// andbT.
      move: mGG'.
