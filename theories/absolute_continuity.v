...
have BU : B `<=` U.
  ...
  by rewrite gtr0_norm ?divr_gt0// gtr_pMr// invf_lt1// ltr1n.
have : B (ratr q) by apply: ballxx; rewrite divr_gt0.
by exists B.
Qed.

Lemma bigcup_ointsub_sup q (I : set R) :
  open I -> is_interval I -> I `<=` U -> I (ratr q) ->
  I `<=` bigcup_ointsub U q.
Proof. by move=> oI itvI IU Iq x Ix; exists I. Qed.
...
- apply: bigcup_ointsub_sup.
  ...
  split; first split.
  + exact: openU.
  + apply/connected_intervalP/connectedU.
    * by exists x.
...
Section open_real_disjoint_intervals.
Context {R : realType}.
Variable U : set R.
...
Definition smaller_idx_disjoint :=
  [set q | forall p, U (ratr p) -> (f p < f q)%N ->
    bigcup_ointsub U q `&` bigcup_ointsub U p = set0].

Let open_disjoint0 : U = \bigcup_(q in smaller_idx_disjoint) bigcup_ointsub U q.
Proof.
...
Definition Irat q :=
  if pselect (smaller_idx_disjoint q) then bigcup_ointsub U q else set0.

Let Irat_trivIset : trivIset [set q | U (ratr q)] Irat.
Proof.
...
Let Irat_set0 q : ratr q \notin U -> Irat q = set0.
Proof.
move=> qU.
rewrite /Irat.
case: pselect => //= H.
rewrite /bigcup_ointsub.
rewrite bigcup0// => B/=[[]oB iB BU].
move/BU.
move=> /mem_set.
by rewrite (negbTE qU).
Qed.

Let open_disjoint1 :
  (forall q, open (Irat q) /\ is_interval (Irat q)) /\
  trivIset [set q | U (ratr q)] Irat /\
  U = \bigcup_q Irat q.
Proof.
split.
  move=> q; split.
    rewrite /Irat; case: pselect => //= _.
      exact: open_bigcup_ointsub.
    exact: open0.
  rewrite /Irat; case: pselect => //= _.
    exact: is_interval_bigcup_ointsub.
...
Let open_disjoint2 :
    (forall q, open (Irat q) /\ is_interval (Irat q)) /\
     trivIset [set n | U (ratr (g n))] (Irat \o g) /\
     U = \bigcup_(q in [set n | U (ratr n)]) Irat q.
Proof.
have [H1 [H2 H3]] := open_disjoint1.
split.
  by [].
split => //.
apply/trivIsetP => i j/= Ugi Ugj ij.
move/trivIsetP : H2 => /(_ (g i) (g j)) /=.
move=> /(_ Ugi Ugj).
apply.
apply: contra ij.
by move/eqP/(can_inj cgf)/eqP.
rewrite [LHS]H3.
...
Definition Irat_total (q : rat) :=
  if (ratr q) \in U then (Irat q) else set0.

Lemma open_disjoint3 :
    (forall q, open (Irat_total q) /\ is_interval (Irat_total q)) /\
     trivIset setT (Irat_total \o g) /\
     U = \bigcup_q Irat_total q.
Proof.
have [H1 [H2 H3]] := open_disjoint2.
split.
  move=> q.
  rewrite /Irat_total.
  case: ifPn => qU.
    ...
    case: pselect => /= H.
    split.
      apply: open_bigcup_ointsub.
    apply: is_interval_bigcup_ointsub.
  split => //.
  split => //.
split => //.
move/trivIset_mkcond: Irat_trivIset => // /trivIsetP H.
...
Lemma open_disjoint : exists I : nat -> set R,
...
exists (Irat_total f \o g).
have [H1 [H2 H3]] := open_disjoint3 cfg cgf.
split.
  by move=> n; exact: H1.
split => //.
rewrite [LHS]H3.
rewrite [LHS](reindex_bigcup g setT)//.
move=> q/= Uq.
by exists (f q).
Qed.
...
Lemma increasing_preimages_gt1 (X Y : set R) (f : R -> R) :
  {in X&, nondecreasing_fun f} ->
  {in (X `\` preimages_gt1 X Y f)& , injective f}.
Proof.
Admitted.
...
- apply/eqP; rewrite -measure_le0/=.
  rewrite -muZ10.
  apply: le_outer_measure.
  apply: (@subset_trans _ (F @^-1` FZ1')).
    by rewrite -image_sub K1K.
  rewrite /FZ1' setDE.
  have -> : F @^-1` ([set F x | x in Z1] `&` ~` preimages_gt1 `[a, b] [set: R] F)
...
apply: HZ.
- apply: (@subset_trans _ (closure Z1)).
    exact: subIsetl.
  rewrite -closure_neitv_oo//.
  exact: closure_subset.
- rewrite /K1.
  ...
  apply: compact_closedI => //.
    ...
    - by rewrite -closure_neitv_oo//; exact: closure_subset.
  rewrite closed_setIS; last exact: interval_closed.
  apply: (continuous_closedP _).1 => //.
  exact: compact_closed.
- apply/eqP; rewrite eq_le; apply/andP; split => //.
  ...
  rewrite -sube_ge0; last first.
    by apply/orP; right; rewrite muZ10 (fin_numE 0)/=.
  rewrite -(setUIDK (closure Z1) Z1).
  rewrite setIidr; last exact: subset_closure.
...
(* transitivity (mu (F @` (K1 `&` Z1))). *)
(*   rewrite -[in LHS](setUIDK K1 Z1). *)
(*   rewrite image_setU. *)
(*   have FK1Z1_gt1 : [set F x | x in K1 `\` Z1] `<=` (preimages_gt1 `[a, b] [set: R] F). *)
(*     admit. *)
...
rewrite -setIA.
  apply: compact_closedI.
  (* have := (continuous_closedP cF).1. *)
    exact: segment_compact.
  rewrite (_ : (F @^-1` K `&` Z1) = `[a, b] `&` F @^-1` (K `&` (F @` Z1))); last first.
    rewrite eqEsubset; split.
      rewrite preimage_setI.
      rewrite subsetI; split.
        rewrite setICA.
        apply: subIset; left; exact: subset_itv_oo_cc.
      apply: setIS.
      exact: preimage_image.
    rewrite preimage_setI setICA.
    rewrite -K1K.
    admit.
  rewrite setIC.
...
(*
  exists (Z1 `|` [set a; b]); split.
  + apply: measurableU; first exact: Gdelta_measurable.
    exact: measurableU.
 admit.
  + rewrite 2?measureU//=; last 4 first.
    * rewrite set1I ifF//.
      apply/negP/negP; rewrite notin_setE/=.
      by apply/eqP; rewrite lt_eqF.
    * exact: Gdelta_measurable.
    * exact: measurableU.
    * rewrite /Z1 setIAC.
      rewrite (_ : `]a, b[ `&` [set a; b] = set0); first by rewrite set0I.
      rewrite setIUr 2!setI1 2?ifF ?setU0//; apply/negP;
          rewrite inE/= in_itv/= ab ltxx//=.
      rewrite 2!lebesgue_measure_set1 2!addr0.
    apply/eqP; rewrite eq_le; apply/andP; split => //.
    rewrite -muZ0.
    rewrite completed_lebesgue_measureE.
    rewrite /lebesgue_measure/lebesgue_stieltjes_measure/measure_extension.
    ...
    apply: le_outer_measure.
    exact: subIsetr.
  + rewrite -(setUIDK K1 [set a; b]) setUC.
    apply: setUSS; last exact: subIsetr.
    apply: (@subset_trans _ (`]a, b[ `&` F @^-1` K)).
      rewrite /K1 -setIDA -setIDAC.
      by rewrite -setDDl setDitv1l setDitv1r.
    rewrite subsetI; split; first exact: subIsetl.
have : set_surj `[a, b] `[F a, F b] F.
  move=> y /= yab.
  have := (@IVT.
    rewrite -K1K.
    move=> x/= [t [/= tab Kt]].
    apply: subset_trans (@image_preimage_subset _ _ F (\bigcap_n U_ n)).
*)
Admitted.
