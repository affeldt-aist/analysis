...
Lemma fsbig_esum (A : set T) a : finite_set A ->
    (forall x, x \in A -> 0 <= a x) ->
...
move=> a_ge0; apply/eqP; rewrite eq_le; apply/andP; split.
  ...
  rewrite fsbig_finite//= big_seq -esum_sum; last first.
    move=> i j _ jX; case: ifPn => // /[!inE] /a_ge0; apply; apply: XI.
    by move: jX; rewrite in_fset_set// inE.
...
apply: (@le_trans _ _
  ...
  rewrite (fsetIidPr _).
    ...
    move=> [x y]/= /imfsetP[/= [x1 y1]] /[!inE] /andP[].
    move=> /imfset2P[x2/=].
    rewrite !inE andbT in_fset_set//; last exact: finite_set_fst.
    move=> /[1!inE] x2X [y2]; rewrite !inE /= => /andP[].
    rewrite in_fset_set; last exact: finite_set_snd.
    move=> /[1!inE] y2X /[1!inE] y2J [-> ->] _ [-> ->].
    by rewrite a_ge0//; move: x2X => [y3] /XIJ [].
