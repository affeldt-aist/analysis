...
suff: ((if P i then op (Ff i x1) (\big[op/x0]_(j <- r | P j) Ff j x1)
        else \big[op/x0]_(j <- r | P j) Ff j x1) @[x1 --> F] -->
       (if P i then op (Fa i) (\big[op/x0]_(j <- r | P j) Fa j)
        else \big[op/x0]_(j <- r | P j) Fa j)) by [].
case: ifPn => // Pi.
