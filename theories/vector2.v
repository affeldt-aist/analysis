...
Require Import boolp classical_sets functions cardinality topology normedtype.
From mathcomp Require Import ring.

Import numFieldTopology.Exports.

(* TODO: we need an all_analysis *)
...
Declare Scope submodule_scope.
Declare Scope submodule_morphism_scope.

Local Open Scope ring_scope.
Local Open Scope classical_set_scope.
(*Local Open Scope submodule_scope.*)
...
(*TODO: mv to classical *)
Coercion pred_of_set_set T (C : set (set T)) (U : C) : {pred T} :=
  fun v => v \in projT1 U.
Canonical set_set_predType T (C : set (set T)) :=
  @PredType _ C (@pred_of_set_set T C).

(*Fact set_set_key T (C : set (set T)) : pred_key (mem C). Proof. by []. Qed.
Definition set_set_keyed T (C : set (set T)) := KeyedPred (@set_set_key _ C).
Canonical set_set_keyed.*)

Module Module.

(* Arbitrary dimension sub-vector space *)
Section Defs.

Definition submodule (R : ringType) (vT : lmodType R) : set (set vT) :=
  [set V : set vT | submod_closed (mem V)(*NB: try with [in V] with 1.16*)].

Definition hom (R : ringType) (vT wT : lmodType R) := {linear vT -> wT}.

End Defs.

Module Exports.

Arguments submodule {R vT}.
Notation "{ 'submodule' vT }" := (@submodule _ vT) : type_scope.
Notation submodule := submodule.
Notation "''Hom' ( aT , rT )" := (hom aT rT) : type_scope.
Notation "''End' ( vT )" := (hom vT vT) : type_scope.

Delimit Scope submodule_scope with SMS.
Bind Scope submodule_scope with submodule. (*NB: this will probably not work*)
(*TODO; bug report that fact that addn is accepted*)

End Exports.
End Module.
Export Module.Exports.

Section submodule_pred.
Variables (R : ringType) (vT : lmodType R).
Implicit Types M : {submodule vT}.

(* mem is a generic key, we put in_set (projT1 M) instead of mem M *)
Fact submodule_key M : pred_key (pred_of_set_set  M(*in_set (projT1 M)*)). Proof. by []. Qed.
Definition submodule_keyed M := KeyedPred (submodule_key M).
Canonical submodule_keyed.

Lemma submod_closed_submodule M : submod_closed (submodule_keyed M).
Proof. by case: M => A inA; exact: set_mem. Qed.

Canonical submodule_opprpred M := OpprPred (submod_closed_submodule M).

Canonical submodule_addrpred M := AddrPred (submod_closed_submodule M).

Canonical submodule_zmodpred M := ZmodPred (@submod_closed_submodule M).

Canonical submodule_submodpred M := SubmodPred (submod_closed_submodule M).
...
Section ModuleDefs.
Variables (R : ringType) (vT : lmodType R).
Implicit Types (u : vT) (X : seq vT) (U V : {submodule vT}).

Definition subsetv (U V : {submodule vT}) := `[< [set` U] `<=` [set` V] >].

Lemma val_submodP u U : reflect (val U u) (u \in U).
Proof.
Admitted.

Check forall u U, u \in U.

Lemma submoduleT : submodule [set: vT].
Proof. by split; [by rewrite inE|move=> *]. Qed.

Definition fullv : {submodule vT} := SigSub (mem_set submoduleT).

Lemma submoduleD (U V : {submodule vT}) :
  submodule [set u.1 + u.2 | u in [set` U] `*` [set` V]].
Proof.
split=> [|k u v] /[!inE].
  by exists (0, 0) => /=; rewrite ?addr0// !rpred0.
(*    apply: rpred0.
    (*apply: (@rpred0 _ _ _ (submodule_keyed U)). (* NB: {pred V} = predPredType V *)*)
  by rewrite rpred0.*)
move=> -[[u1 u2] [/=  hu1  hu2] <-{u}] [[v1 v2] [/=  hv1  hv2] <-{v}].
exists (k *: u1 + v1 ,(*NB: notation bug rm space*) k *: u2 + v2) => /=; last by rewrite !scalerDr addrACA.
by split; rewrite rpredD// rpredZ.
Qed.

Definition addv U V : {submodule vT} := SigSub (mem_set (submoduleD U V)).

Lemma submoduleI (U V : {submodule vT}) : submodule ([set` U] `&` [set` V]).
Proof.
Admitted.

Definition capv U V : {submodule vT} := SigSub (mem_set (submoduleI U V)).

Lemma submoduleC (U : {submodule vT}) : submodule (~` [set` U]).
Proof.
Admitted.

Definition complv U : {submodule vT} := SigSub (mem_set (submoduleC U)).

Lemma submodule_setD (U V : {submodule vT}) : submodule ([set` U] `\` [set` V]).
Proof.
Admitted.

Definition diffv U V : {submodule vT} := SigSub (mem_set (submodule_setD U V)).

Definition vpick (U : {submodule vT}) := xget 0 ([set` U] `\ 0).
(* TODO lmodtype pointed type to write get ([set` U] `\ 0) *)

Fact span_key : unit. Proof. by []. Qed.

Section fun_set0.
Context (aT rT : Type) {X : set rT}.

Definition fun_set0 (f : aT -> rT) := f.

Lemma fun0P (f : aT -> rT) : @IsFun aT rT set0 X (fun_set0 f).
Proof. by constructor. Qed.

HB.instance Definition _ (f : aT -> rT)  := (*IsFun.Build _ _ _ _ (fun=> 0)*) (fun0P f).

End fun_set0.

Definition span_set (X : set vT) : set vT :=
  \bigcup_n [set \sum_(i < n) k i *: v i | k in [set: nat -> R] &
                                           v in [set: {fun `I_n >-> X}]].
(* we pick a function for v because the codomain of v is a non-trivial subset
  while the codomain of k is the full set, that is why we do not bother
  fixing the domain of k with a "function with domain and codomain" from
  functions.v for k *)
Lemma submodule_span_set (X : set vT) : submodule (span_set X).
Proof.
split.
  rewrite inE.
  (*Print Canonical Projections Pointed.sort.
  have := @point (numFieldTopology.pointed_ringType R).
  have := [pointedType of R].*) (* try to put a structure of pointed on ring *)
  exists 0%N => //=.
  exists (fun=> 0) => //.
(*  exists (fun=>point) => //. (* TODO: use point instead *)*)
  rewrite II0.
  exists [fun of (fun_set0 (cst 0))] => //.
  by rewrite big_ord0.
move=> k u v.
Admitted.

Definition span_expanded_def (X : set vT) : {submodule vT} := SigSub (mem_set (submodule_span_set X)).
...
Definition free (X : set vT) :=
  forall (n : nat) (v : {injfun `I_n >-> X})
         (k : {fun `I_n >-> [set: R]}), (0 < n)%N ->
    \sum_(i < n) k i *: v i = 0 -> forall i, k i = 0.

Lemma freeP (X : set vT) : free X <->
  forall (n : nat) (v : {injfun `I_n >-> X})
         (k : {fun `I_n >-> [set: R]}), (0 < n)%N ->
    \sum_(i < n) k i *: v i = 0 -> forall i, k i = 0.
Proof. by []. Qed.

Lemma freePn (X : set vT) : ~ free X <->
  exists (n : nat) (v : {injfun `I_n.+1 >-> X})
         (k : {fun `I_n.+1 >-> [set: R] `\ 0}),
    \sum_(i < n.+1) k i *: v i = 0.
Proof.
Admitted.

Definition basis_of (U : {submodule vT}) : set (set vT) :=
  [set X : set vT | span X = U /\ free X].

End ModuleDefs.
Notation "U <= V" := (subsetv U V) : submodule_scope.

Section VspaceDefs.
Variables (K : fieldType) (vT : lmodType K).

Definition complete_basis (U : {submodule vT}) (X : set vT) : set vT :=
  [get Y : set vT | X `<=` Y /\ basis_of U Y ].

Lemma complete_basisP (U : {submodule vT}) (X : set vT) :
    free X -> (span X <= U)%SMS ->
  basis_of U (complete_basis U X).
(* TODO: use Zorn's lemma*)
Admitted.

Lemma complete_basisS (U : {submodule vT}) (X : set vT) :
    free X -> (span X <= U)%SMS ->
  X `<=` complete_basis U X.
Admitted.

Definition basis (U : {submodule vT}) : set vT :=
  complete_basis U set0.
(*Definition vbasis_def U :=
  [tuple r2v (row i (row_base (vs2mx U))) | i < dimv U].*)

Definition finite_dim (U : {submodule vT}) := finite_set (basis U).

Definition trimmed_basis (U : {submodule vT}) (X : set vT) : set vT :=
  [get Y : set vT | Y `<=` X /\ basis_of U Y ].

Lemma trimmed_basisP (U : {submodule vT}) (X : set vT) :
  (span X = U) -> basis_of U (trimmed_basis U X).
Admitted.

Lemma trimmed_basisS (U : {submodule vT}) (X : set vT) :
  (span X = U)%SMS -> trimmed_basis U X `<=` X.
Admitted.


(*Definition vbasis := locked_with span_key vbasis_def.*)
(*compute a basis from a submodule*)
...
Definition dimv (U : {submodule vT}) : nat := #|` fset_set (basis U) |.
(* TODO: define the type of extended real number *)

(* TODO: put a lattice type on submodules *)

(*Definition vline (v : vT) := (span [set v]).*)

End VspaceDefs.

(*Coercion pred_of_vspace : Vector.space >-> pred_sort.*)(*same as pred_of_set_set*)
Notation "\dim U" := (dimv U%SMS) : nat_scope.
Notation "U <= V" := (subsetv U%SMS V%SMS) : submodule_scope.
Notation "U <= V <= W" := ((U <= V)%SMS && (V <= W)%SMS) : submodule_scope.
Notation "<< X >>" := (span X) : submodule_scope.
Notation "<[ v ]>" := (<< [set v] >>)%SMS : submodule_scope.
Notation "0" := (<[ 0 ]>)%SMS : submodule_scope.

Arguments fullv {R vT}.
Prenex Implicits subsetv addv capv complv diffv span free basis_of.

Notation "U + V" := (addv U V) : submodule_scope.
Notation "U :&: V" := (capv U V) : submodule_scope.
Notation "U ^C" := (complv U) (at level 8, format "U ^C") : submodule_scope.
Notation "U :\: V" := (diffv U V) : submodule_scope.
Notation "{ : vT }" := (@fullv _ vT) (only parsing) : submodule_scope.

Notation "\sum_ ( i <- r | P ) U" :=
  (\big[addv/0%SMS]_(i <- r | P%B) U%SMS) : submodule_scope.
Notation "\sum_ ( i <- r ) U" :=
  (\big[addv/0%SMS]_(i <- r) U%SMS) : submodule_scope.
Notation "\sum_ ( m <= i < n | P ) U" :=
  (\big[addv/0%SMS]_(m <= i < n | P%B) U%SMS) : submodule_scope.
Notation "\sum_ ( m <= i < n ) U" :=
  (\big[addv/0%SMS]_(m <= i < n) U%SMS) : submodule_scope.
Notation "\sum_ ( i | P ) U" :=
  (\big[addv/0%SMS]_(i | P%B) U%SMS) : submodule_scope.
Notation "\sum_ i U" :=
  (\big[addv/0%SMS]_i U%SMS) : submodule_scope.
Notation "\sum_ ( i : t | P ) U" :=
  (\big[addv/0%SMS]_(i : t | P%B) U%SMS) (only parsing) : submodule_scope.
Notation "\sum_ ( i : t ) U" :=
  (\big[addv/0%SMS]_(i : t) U%SMS) (only parsing) : submodule_scope.
Notation "\sum_ ( i < n | P ) U" :=
  (\big[addv/0%SMS]_(i < n | P%B) U%SMS) : submodule_scope.
Notation "\sum_ ( i < n ) U" :=
  (\big[addv/0%SMS]_(i < n) U%SMS) : submodule_scope.
Notation "\sum_ ( i 'in' A | P ) U" :=
  (\big[addv/0%SMS]_(i in A | P%B) U%SMS) : submodule_scope.
Notation "\sum_ ( i 'in' A ) U" :=
  (\big[addv/0%SMS]_(i in A) U%SMS) : submodule_scope.

Notation "\bigcap_ ( i <- r | P ) U" :=
  (\big[capv/fullv]_(i <- r | P%B) U%SMS) : submodule_scope.
Notation "\bigcap_ ( i <- r ) U" :=
  (\big[capv/fullv]_(i <- r) U%SMS) : submodule_scope.
Notation "\bigcap_ ( m <= i < n | P ) U" :=
  (\big[capv/fullv]_(m <= i < n | P%B) U%SMS) : submodule_scope.
Notation "\bigcap_ ( m <= i < n ) U" :=
  (\big[capv/fullv]_(m <= i < n) U%SMS) : submodule_scope.
Notation "\bigcap_ ( i | P ) U" :=
  (\big[capv/fullv]_(i | P%B) U%SMS) : submodule_scope.
Notation "\bigcap_ i U" :=
  (\big[capv/fullv]_i U%SMS) : submodule_scope.
Notation "\bigcap_ ( i : t | P ) U" :=
  (\big[capv/fullv]_(i : t | P%B) U%SMS) (only parsing) : submodule_scope.
Notation "\bigcap_ ( i : t ) U" :=
  (\big[capv/fullv]_(i : t) U%SMS) (only parsing) : submodule_scope.
Notation "\bigcap_ ( i < n | P ) U" :=
  (\big[capv/fullv]_(i < n | P%B) U%SMS) : submodule_scope.
Notation "\bigcap_ ( i < n ) U" :=
  (\big[capv/fullv]_(i < n) U%SMS) : submodule_scope.
Notation "\bigcap_ ( i 'in' A | P ) U" :=
  (\big[capv/fullv]_(i in A | P%B) U%SMS) : submodule_scope.
Notation "\bigcap_ ( i 'in' A ) U" :=
  (\big[capv/fullv]_(i in A) U%SMS) : submodule_scope.

Section VectorTheory.
Variables (K : fieldType) (vT : lmodType K).
Implicit Types (a : K) (u v w : vT) (X Y : seq vT) (U V W : {submodule vT}).
...
Lemma subvP U V : reflect {subset U <= V} (U <= V)%SMS.
(*Lemma subsetvP U V : reflect ([set` U] `<=` [set` V]) (U <= V)%SMS.*)
Proof.
Admitted.

Lemma memvE v U : (v \in U) = (<[v]> <= U)%SMS.
Proof.
apply/idP/subvP.
  rewrite inE// => Uv x/=. (*this *has* to be correct*)
Admitted.

Lemma vlineP v1 v2 : reflect (exists k, v1 = k *: v2) (v1 \in <[v2]>)%SMS.
Proof.
(*apply: (iffP idP) => [|[k ->]]; rewrite memvK genmxE ?linearZ ?scalemx_sub //.
by case/sub_rVP=> k; rewrite -linearZ => /v2r_inj->; exists k.
Qed.*) Admitted.

Lemma mem0v U : 0%R \in U. Proof. exact: rpred0. Qed.
...
Lemma memv_line u : u \in <[u]>%SMS.
Proof. by apply/vlineP; exists 1; rewrite scale1r. Qed.

Lemma subvv U : (U <= U)%SMS. Proof. exact/subvP. Qed.
...
Proof.
Admitted.
(* by move=> U V; apply/vs2mxP. Qed.*)

Lemma eqEsubv U V : (U == V) = (U <= V <= U)%SMS.
...
(* Empty space. *)
Lemma sub0v U : (0 <= U)%SMS.
Proof.
apply/subvP => x /[!inE].


Proof. (*exact: mem0v. Qed.*) Admitted.

Lemma subv0 U : (U <= 0)%SMS = (U == 0%SMS).
Proof. by rewrite eqEsubv sub0v andbT. Qed.

Lemma memv0 v : v \in 0%SMS = (v == 0).
Proof. by apply/idP/eqP=> [/vlineP[k ->] | ->]; rewrite (scaler0, mem0v). Qed.

(* Full space *)

Lemma vline0 : [set` 0%SMS] = [set 0] :> set vT.
Proof.
Admitted.
...
Lemma subvf U : (U <= fullv)%SMS.
Proof.
by apply/subvP => x _ /[!inE].
Qed.
...
Proof.
Admitted.
(*by rewrite mem_r2v nz_row_sub. Qed.*)

Lemma vpick0 U : (vpick U == 0) = (U == 0%SMS).
Proof.
Admitted.
(*by  rewrite -memv0 mem_r2v -subv0 /subV vs2mx0 !submx0 nz_row_eq0. Qed.*)

(* Sum of subspaces. *)
Lemma subv_add U V W : (U + V <= W)%SMS = (U <= W)%SMS && (V <= W)%SMS.
Proof. (*by rewrite /subV vs2mxD addsmx_sub. Qed.*) Admitted.

Lemma addvS U1 U2 V1 V2 : (U1 <= U2 -> V1 <= V2 -> U1 + V1 <= U2 + V2)%SMS.
Proof. (*by rewrite /subV !vs2mxD; apply: addsmxS. Qed.*) Admitted.

Lemma addvSl U V : (U <= U + V)%SMS.
Proof. (*by rewrite /subV vs2mxD addsmxSl. Qed.*)Admitted.

Lemma addvSr U V : (V <= U + V)%SMS.
Proof. (*by rewrite /subV vs2mxD addsmxSr. Qed.*)Admitted.

Lemma addvC : commutative addV.
Proof. (*by move=> U V; apply/vs2mxP; rewrite !vs2mxD addsmxC submx_refl. Qed.*)Admitted.

Lemma addvA : associative addV.
Proof. (*by move=> U V W; apply/vs2mxP; rewrite !vs2mxD addsmxA submx_refl. Qed.*) Admitted.

Lemma addv_idPl {U V}: reflect (U + V = U)%SMS (V <= U)%SMS.
Proof. (*by rewrite /subV (sameP addsmx_idPl eqmxP) -vs2mxD; apply: vs2mxP. Qed.*) Admitted.

Lemma addv_idPr {U V} : reflect (U + V = V)%SMS (U <= V)%SMS.
Proof. by rewrite addvC; apply: addv_idPl. Qed.

Lemma addvv : idempotent addV.
Proof. by move=> U; apply/addv_idPl. Qed.
 
Lemma add0v : left_id 0%SMS addV.
Proof. by move=> U; apply/addv_idPr/sub0v. Qed.

Lemma addv0 : right_id 0%SMS addV.
...
Lemma memv_add u v U V : u \in U -> v \in V -> u + v \in (U + V)%SMS.
Proof.
(* by rewrite !memvK genmxE linearD; apply: addmx_sub_adds. Qed.*) Admitted.
...
Lemma memv_sumr P vs (Us : I -> {submodule vT}) :
    (forall i, P i -> vs i \in Us i) ->
  \sum_(i | P i) vs i \in (\sum_(i | P i) Us i)%SMS.
Proof.
move=> Uv; apply/rpred_sum=> i Pi.
(*apply/(sumv_sup i Pi)/Uv. Qed.*) Admitted.

Lemma memv_sumP {P} {Us : I -> {submodule vT}} {v} : 
  reflect (exists2 vs, forall i, P i ->  vs i \in Us i
                     & v = \sum_(i | P i) vs i)
          (v \in \sum_(i | P i) Us i)%SMS.
Proof.
...
Lemma subv_cap U V W : (U <= V :&: W)%SMS = (U <= V)%SMS && (U <= W)%SMS.
Proof. (*by rewrite /subV vs2mxI sub_capmx. Qed.*) Admitted.

Lemma capvS U1 U2 V1 V2 : (U1 <= U2 -> V1 <= V2 -> U1 :&: V1 <= U2 :&: V2)%SMS.
Proof. (*by rewrite /subV !vs2mxI; apply: capmxS. Qed.*) Admitted.

Lemma capvSl U V : (U :&: V <= U)%SMS.
Proof. (*by rewrite /subV vs2mxI capmxSl. Qed.*) Admitted.

Lemma capvSr U V : (U :&: V <= V)%SMS.
Proof. (*by rewrite /subV vs2mxI capmxSr. Qed.*) Admitted.

Lemma capvC : commutative capV.
Proof. (*by move=> U V; apply/vs2mxP; rewrite !vs2mxI capmxC submx_refl. Qed.*) Admitted.

Lemma capvA : associative capV.
Proof. (*by move=> U V W; apply/vs2mxP; rewrite !vs2mxI capmxA submx_refl. Qed.*) Admitted.

Lemma capv_idPl {U V} : reflect (U :&: V = U)%SMS (U <= V)%SMS.
Proof. (*by rewrite /subV(sameP capmx_idPl eqmxP) -vs2mxI; apply: vs2mxP. Qed.*) Admitted.

Lemma capv_idPr {U V} : reflect (U :&: V = V)%SMS (V <= U)%SMS.
Proof. by rewrite capvC; apply: capv_idPl. Qed.

Lemma capvv : idempotent capV.
Proof. by move=> U; apply/capv_idPl. Qed.

Lemma cap0v : left_zero 0%SMS capV.
Proof. by move=> U; apply/capv_idPl/sub0v. Qed.

Lemma capv0 : right_zero 0%SMS capV.
...
Lemma memv_cap w U V : (w \in U :&: V)%SMS = (w \in U) && (w \in V).
Proof. by rewrite !memvE subv_cap. Qed.

Lemma memv_capP {w U V} : reflect (w \in U /\ w \in V) (w \in U :&: V)%SMS.
Proof. by rewrite memv_cap; apply: andP. Qed.

Lemma vspace_modl U V W : (U <= W -> U + (V :&: W) = (U + V) :&: W)%SMS.
Proof.
(*by move=> sUV; apply/vs2mxP; rewrite !(vs2mxD, vs2mxI); apply/eqmxP/matrix_modl.
Qed.*) Admitted.

Lemma vspace_modr  U V W : (W <= U -> (U :&: V) + W = U :&: (V + W))%SMS.
...
Lemma bigcapv_inf i0 P Us V :
  P i0 -> (Us i0 <= V -> \bigcap_(i | P i) Us i <= V)%SMS.
Proof. by move=> Pi0; apply: subv_trans; rewrite (bigD1 i0) ?capvSl. Qed.

Lemma subv_bigcapP {P U Vs} :
  reflect (forall i, P i -> U <= Vs i)%SMS (U <= \bigcap_(i | P i) Vs i)%SMS.
Proof.
apply: (iffP idP) => [sUV i Pi | sUV].
...
(* Difference *)
Lemma diffvSl U V : (U :\: V <= U)%SMS.
Proof. (*by rewrite /subV genmxE diffmxSl. Qed.*) Admitted.

Lemma capv_diff U V : ((U :\: V) :&: V = 0)%SMS.
Proof.
(*apply/val_inj; rewrite [val]/= vs2mx0 vs2mxI -(gen_vs2mx V) -genmx_cap.
by rewrite capmx_diff genmx0.*) Admitted.

Lemma addv_diff_cap U V : (U :\: V + U :&: V)%SMS = U.
Proof.
(*apply/vs2mxP; rewrite vs2mxD -genmx_adds !genmxE.
exact/eqmxP/addsmx_diff_cap_eq.
Qed.*)Admitted.

Lemma addv_diff U V : (U :\: V + V = U + V)%SMS.
Proof. by rewrite -{2}(addv_diff_cap U V) -addvA (addv_idPr (capvSr U V)). Qed.

(* Subspace dimension. *)
Lemma dimv0 : \dim (0%SMS : {submodule vT}) = 0%N.
Proof. (*by rewrite /dimv vs2mx0 mxrank0. Qed.*) Admitted.

Lemma dimv_eq0 U : reflect (finite_dim U -> U = 0%SMS) (\dim U == 0%N).
Proof.
...
Proof. (*by rewrite /dimv mxrank_gen rank_rV (can2_eq v2rK r2vK) linear0. Qed.*) Admitted.

Local Open Scope card_scope.

Lemma le_cardv U V : (U <= V)%SMS -> basis U #<= basis V.
Proof. (*exact: mxrankS. Qed.*) Admitted.

Lemma dimvS U V : finite_dim V -> (U <= V)%SMS -> \dim U <= \dim V.
Proof. (*exact: mxrankS. Qed.*) Admitted. (* consequence of le_cardv *)



Lemma dimv_leqif_sup U V : (U <= V)%SMS -> \dim U <= \dim V ?= iff (V <= U)%SMS.
Proof. (*exact: mxrank_leqif_sup. Qed.*) Admitted.

Lemma dimv_leqif_eq U V : finite_dim V -> (U <= V)%SMS -> \dim U <= \dim V ?= iff (U == V).
Proof. (*by rewrite eqEsubv; apply: mxrank_leqif_eq. Qed.*) Admitted.

Lemma eqEdim U V : finite_dim V -> (U == V) = (U <= V)%SMS && (\dim V <= \dim U).
Proof. by move=> dimV; apply/idP/andP=> [/eqP | [/dimv_leqif_eq/geq_leqif]] ->. Qed.

Lemma dimv_compl U : finite_dim (fullv : {submodule vT}) -> \dim U^C = (\dim {:vT} - \dim U)%N.
...
Local Notation directv A := (directv_def (Phantom {vspace _} A%SMS)).
...
Lemma directv_addE (S1 S2 : addv_expr) :
  directv (unwrap S1 + unwrap S2)
    = [&& directv (unwrap S1), directv (unwrap S2)
        & unwrap S1 :&: unwrap S2 == 0]%SMS.
Proof.
by rewrite /directv_def /dimv vs2mxD -mxdirectE mxdirect_addsE -vs2mxI -vs2mx0.
Qed.

Lemma directv_addP {U V} : reflect (U :&: V = 0)%SMS (directv (U + V)).
...
Lemma directv_sumP {Us : I -> {vspace vT}} :
  reflect (forall i, P i -> Us i :&: (\sum_(j | P j && (j != i)) Us j) = 0)%SMS
...
Lemma span_nil : (<<Nil vT>> = 0)%SMS.
Proof. by rewrite span_def big_nil. Qed.

Lemma span_seq1 v : (<<[:: v]>> = <[v]>)%SMS.
Proof. by rewrite span_def big_seq1. Qed.

Lemma span_cons v X : (<<v :: X>> = <[v]> + <<X>>)%SMS.
Proof. by rewrite !span_def big_cons. Qed.

Lemma span_cat X Y : (<<X ++ Y>> = <<X>> + <<Y>>)%SMS.
...
Lemma freeNE n (X : n.-tuple vT) :
  ~~ free X = [exists i : 'I_n, X`_i \in <<drop i.+1 X>>%SMS].
...
Lemma span_basis U X : basis_of U X -> <<X>>%SMS = U.
...
Lemma basisEdim X U : basis_of U X = (U <= <<X>>)%SMS && (size X <= \dim U).
Proof.
...
Lemma basisEfree X U :
  basis_of U X = [&& free X, (<<X>> <= U)%SMS & \dim U <= size X].
Proof.
...
Lemma span_bigcat :
  (<<\big[cat/[::]]_(i | P i) Xs i>> = \sum_(i | P i) <<Xs i>>)%SMS.
...
Lemma bigcat_basis Us (U := (\sum_(i | P i) Us i)%SMS) :
...
Notation directv S := (directv_def (Phantom _ S%SMS)).
...
Definition lfun_preim aT rT (f : 'Hom(aT, rT)) W :=
  (lfun_img (inv_lfun f) (W :&: lfun_img f fullv) + lker f)%SMS.
...
suffices /subvPn[_ /memv_imgP[u _ ->]]: ~~ (limg (f - g) <= 0)%SMS.
...
Lemma lkerE f U : (U <= lker f)%SMS = (f @: U == 0)%SMS.
Proof.
...
Lemma eqlfun_inP V f g : reflect {in V, f =1 g} (V <= lker (f - g))%SMS.
Proof. by apply: (iffP subvP) => E x /E/eqlfunP. Qed.

Lemma limg_ker_compl f U : (f @: (U :\: lker f) = f @: U)%SMS.
Proof.
...
Lemma limg_ker0 f U V : lker f == 0%SMS -> (f @: U <= f @: V)%SMS = (U <= V)%SMS.
Proof.
...
Lemma eq_limg_ker0 f U V : lker f == 0%SMS -> (f @: U == f @: V)%SMS = (U == V).
Proof. by move=> injf; rewrite !eqEsubv !limg_ker0. Qed.

Lemma lker0_lfunK f : lker f == 0%SMS -> cancel f f^-1%VF.
Proof.
by move/lker0P=> injf u; apply: injf; rewrite limg_lfunVK ?memv_img ?memvf.
Qed.

Lemma lker0_compVf f : lker f == 0%SMS -> (f^-1 \o f = \1)%VF.
...
Lemma fixedSpacesP f U : reflect {in U, f =1 id} (U <= fixedSpace f)%SMS.
Proof. by apply: (iffP subvP) => cUf x /cUf/fixedSpaceP. Qed.

Lemma fixedSpace_limg f U : (U <= fixedSpace f -> f @: U = U)%SMS.
Proof.
move/fixedSpacesP=> cUf; apply/vspaceP=> x.
by apply/memv_imgP/idP=> [[{}x Ux ->] | Ux]; last exists x; rewrite ?cUf.
Qed.

Lemma fixedSpace_id : fixedSpace \1 = {:vT}%SMS.
Proof.
...
Lemma lim1g U : (\1 @: U)%SMS = U.
Proof.
have /andP[/eqP <- _] := vbasisP U; rewrite limg_span map_id_in // => u _.
by rewrite lfunE.
Qed.

Lemma limg_comp f g U : ((g \o f) @: U = g @: (f @: U))%SMS.
Proof.
...
Lemma lpreim_cap_limg f W : (f @^-1: (W :&: limg f))%SMS = (f @^-1: W)%SMS.
Proof. by rewrite /lfun_preim -capvA capvv. Qed.

Lemma lpreim0 f : (f @^-1: 0)%SMS = lker f.
Proof. by rewrite /lfun_preim cap0v limg0 add0v. Qed.

Lemma lpreimS f V W : (V <= W)%SMS-> (f @^-1: V <= f @^-1: W)%SMS.
Proof. by move=> sVW; rewrite addvS // limgS // capvS. Qed.

Lemma lpreimK f W : (W <= limg f)%SMS -> (f @: (f @^-1: W))%SMS = W.
Proof.
move=> sWf; rewrite limgD (capv_idPl sWf) // -limg_comp.
have /eqP->: (f @: lker f == 0)%SMS by rewrite -lkerE.
...
Lemma memv_preim f u W : (f u \in W) = (u \in f @^-1: W)%SMS.
Proof.
...
Lemma daddv_pi_id U V u : (U :&: V = 0)%SMS -> u \in U -> daddv_pi U V u = u.
Proof.
...
Lemma daddv_pi_proj U V w (pi := daddv_pi U V) :
  (U :&: V = 0)%SMS -> pi (pi w) = pi w.
Proof. by move/daddv_pi_id=> -> //; apply: memv_pi. Qed.

Lemma daddv_pi_add U V w :
  (U :&: V = 0)%SMS -> (w \in U + V)%SMS -> daddv_pi U V w + daddv_pi V U w = w.
Proof.
move/eqP; rewrite -dimv_eq0 memvE /subsetv /dimv !genmxE mxrank_eq0 => /eqP.
...
Lemma memv_projC U w : w - projv U w \in (U^C)%SMS.
Proof.
...
Lemma lker_proj U : lker (projv U) = (U^C)%SMS.
Proof.
...
Lemma addv_pi1_pi2 U V w :
  w \in (U + V)%SMS -> addv_pi1 U V w + addv_pi2 U V w = w.
...
Let sumv_pi_rec i :=
  fix loop r := if r is j :: r1 then
    let V1 := (\sum_(k <- r1) Vs k)%SMS in
    if j == i then addv_pi1 (Vs j) V1 else (loop r1 \o addv_pi2 (Vs j) V1)%VF
  else 0.

Notation sumV := (\sum_(i <- r0 | P i) Vs i)%SMS.
...
Lemma sumv_pi_nat_sum m n (P : pred nat) Vs v (V : {vspace vT})
                      (defV : V = (\sum_(m <= i < n | P i) Vs i)%SMS) :
...
Definition vsolve_eq U := finfun (tnth rhs) \in (linfun lhsf @: U)%SMS.
