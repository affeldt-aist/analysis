...
Proof.
by rewrite /normal_pdf; case: ifP => // _; exact: normal_pdf0_ge0.
Qed.
...
(* NB: (sqrtr 0)^-1 = 0 < 1 *)
Lemma normal_pdf_ub m s x : s != 0 -> normal_pdf m s x <= normalpi s.
Proof.
...
rewrite integralZl//=.
  by rewrite integral_normalexp// -EFinM divff// gt_eqF// normalpi_gt0.
apply/integrableP; split.
  by apply/measurable_EFinP; exact: measurable_normalexp.
under eq_integral.
  move=> /= x _; rewrite ger0_norm ?expR_ge0//; over.
by rewrite integral_normalexp// ltry.
Qed.
...
apply/abse_integralP => //=.
  by apply/measurable_EFinP; exact: measurable_normal_pdf.
by rewrite integral_normal_pdf abse1 ltry.
Qed.
...
apply: (@le_trans _ _ (\int[mu]_(x in A) (normalpi sigma)%:E))%E; last first.
  by rewrite integral_cst//= muA0 mule0.
apply: ge0_le_integral => //=.
- by move=> x _; rewrite lee_fin normal_pdf0_ge0.
- apply/measurable_funTS/measurableT_comp => //.
  exact: measurable_normal_pdf0.
- by move=> x _; rewrite lee_fin normalpi_ge0.
...
- move=> y _ x.
  under [X in _ _ X]eq_fun.
    move=> x0.
    rewrite normal_pdfE// /normal_pdf0 /normalexp -(sqrrN (y - _)) opprB.
    over.
  exact: continuous_normal_pdf0.
- apply: (integrableS measurableT) => //=.
