...
Notation "% x" := (@exp_var _ _ x _ erefl)
  (in custom expr at level 1, format "% x") : lang_scope.
Notation "e :+ x" := (@exp_weak _ _ [::] _ _ (x, _) e erefl)
  (in custom expr at level 1) : lang_scope.
...
- move=> g t e0 k ev IH {}v {}mv.
  ...
  inj_ex H4; subst v.
  by rewrite (IH _ H3).
- move=> g t e1 f1 mf1 e2 f2 mf2 e3 f3 mf3 ev1 IH1 ev2 IH2 ev3 IH3 f mf.
  inversion 1; subst g0 t0.
  inj_ex H2; subst e0.
  inj_ex H6; subst e6.
  inj_ex H7; subst e7.
  inj_ex H9; subst f.
  clear H11.
  have ? := IH1 _ _ H5; subst f6.
  have ? := IH2 _ _ H12; subst f7.
  by have ? := IH3 _ _ H13; subst f8.
...
- move=> g t e k ev IH {}v {}mv.
  ...
  inj_ex H5; subst mv.
  by rewrite (IH _ H3).
- move=> g t e1 f1 mf1 e2 f2 mf2 e3 f3 mf3 ev IH ev1 IH1 ev2 IH2 f mf.
  inversion 1; subst g0 t0.
  inj_ex H2; subst e0.
  inj_ex H6; subst e6.
  inj_ex H7; subst e7.
  inj_ex H9; subst f.
  clear H11.
  have ? := IH1 _ _ H12; subst f7.
  have ? := IH2 _ _ H13; subst f8.
  by have ? := IH _ _ H5; subst f6.
...
Proof.
split.
  rewrite /execD; case: cid => x' [mx' H] [?]; subst x'.
  have ? : mx = mx' by [].
  by subst mx'.
rewrite /execD; case: cid => f' [mf' f'mf']/=.
...
Proof.
split.
  by move=> <-; rewrite /execP; case: cid.
rewrite /execP; case: cid => // x0 Hx0.
...
Lemma execD_var g str : let i := index str (map fst g) in
  @execD g _ (exp_var str erefl) = existT _ (@acc_typ R (map snd g) i)
                                   (measurable_acc_typ (map snd g) i).
