...
Definition measure_of_smeasure d (T : measurableType d) (R : realType)
...
HB.instance Definition _ := isMeasure.Build d R T (measure_of_smeasure nupos)
...
Definition sjordan_pos : {smeasure set X -> \bar R} :=
  [the smeasure _ _ of smrestr nu mP].
...
Definition sjordan_neg : {smeasure set X -> \bar R} :=
  [the smeasure _ _ of smscale (-1) [the smeasure _ _ of smrestr nu mN]].
...
Definition jordan_pos : {measure set X -> \bar R} :=
  [the measure _ _ of measure_of_smeasure _ positive_set_sjordan_pos].

Definition jordan_neg : {measure set X -> \bar R} :=
  [the measure _ _ of measure_of_smeasure _ positive_set_sjordan_neg].

Lemma finite_jordan_pos : finite_measure jordan_pos.
Proof.
rewrite /finite_measure /jordan_pos/= /smrestr/=.
by rewrite /measure_of_smeasure setTI fin_num_ltey// isfinite.
Qed.

Lemma finite_jordan_neg : finite_measure jordan_neg.
Proof.
rewrite /finite_measure /jordan_neg/= /smscale/=.
rewrite /measure_of_smeasure EFinN mulN1e /smrestr setTI.
...
by rewrite /measure_of_smeasure EFinN mulN1e oppeK.
Qed.
...
rewrite /measure_of_smeasure EFinN mulN1e oppeK.
...
have muAP_gt0 : 0 < mu AP.
  ...
  rewrite (@lt_le_trans _ _ (sigma AP))//.
    rewrite (@lt_le_trans _ _ (sigma A))//; last first.
      rewrite (smeasure_partition _ _ mP mN)// gee_addl//.
...
pose mu_ j : {measure set X -> \bar R} := [the measure _ _ of mrestr mu (mE j)].
pose nu_ j : {measure set X -> \bar R} := [the measure _ _ of mrestr nu (mE j)].
