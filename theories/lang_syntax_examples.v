...
Lemma integral_Beta_bernoulli_lty a b U :
  (\int[Beta a b]_x `|bernoulli (x : R) U| < +oo)%E.
Proof.
...
Lemma integrable_bernoulli_ubeta_pdf a b U
  (mu : {measure set (g_sigma_algebraType R.-ocitv.-measurable) -> \bar R}) :
  measurable U -> (mu `[0%R, 1%R]%classic < +oo)%E ->
  mu.-integrable `[0, 1] (fun x => bernoulli (ubeta_pdf a b x) U).
Proof.
move=> mU mu01oo.
apply/integrableP; split.
  apply: (measurableT_comp (measurable_bernoulli2 _)) => //=.
  by apply: measurable_funTS; exact: measurable_ubeta_pdf.
...
Let measurable_bernoulli_ubeta_pdf U :
   measurable_fun setT (fun x : R => bernoulli (ubeta_pdf 2 1 x) U).
Proof.
apply: (measurableT_comp (measurable_bernoulli2 _)) => //=.
exact: measurable_ubeta_pdf.
Qed.
...
transitivity (Beta_bernoulli 6 4 1 0 U : \bar R).
  rewrite /Beta_bernoulli !letin'E/=.
  rewrite integral_Beta//=; last 2 first.
    exact: measurable_bernoulli2.
    exact: integral_Beta_bernoulli_lty.
  rewrite integral_Beta//=; last 2 first.
    by apply: measurable_funTS => /=; exact: measurable_bernoulli_ubeta_pdf.
    rewrite integral_Beta//=.
    + suff: mu.-integrable `[0%R, 1%R]
          (fun x => bernoulli (ubeta_pdf 2 1 x) U * (beta_pdf 6 4 x)%:E)%E.
        move=> /integrableP[_].
        under eq_integral.
          move=> x _.
    ...
    + apply: integrableMl => //=.
      * apply: integrable_bernoulli_ubeta_pdf => //=.
    ...
    + apply/measurableT_comp => //.
      ...
      exact: measurable_bernoulli_ubeta_pdf.
    + under eq_integral do rewrite gee0_abs//=.
      have : (Beta 6 4 `[0%R, 1%R] < +oo :> \bar R)%E.
        by rewrite -ge0_fin_numE// Beta_fin_num.
      by move=> /(@integrable_bernoulli_ubeta_pdf 2 1 _ (Beta 6 4) mU) /integrableP[].
  rewrite [RHS]integral_mkcond.
  apply: eq_integral => x _ /=.
  rewrite patchE.
  case: ifPn => x01.
    by rewrite /beta_pdf /ubeta_pdf !patchE x01/= XnMonemXn0' expr1.
  by rewrite /beta_pdf /ubeta_pdf patchE (negbTE x01) mul0r mule0.
rewrite Beta_bernoulliE// !bernoulliE//=; last 2 first.
  lra.
  by rewrite div_beta_norm_ge0 div_beta_norm_le1.
congr (_ * _ + _ * _)%:E.
  by rewrite /div_beta_norm/= !int_ubeta_pdfE/= !factE/=; field.
by rewrite /onem /div_beta_norm !int_ubeta_pdfE/= !factE/=; field.
Qed.
