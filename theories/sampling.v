...
(* add to classical_sets as a generalization of subUset *)
Lemma subbigcupset (I : set nat) T A (B : nat -> set T) :
  (\bigcup_(k in I) B k `<=` A) = (forall k, I k -> B k `<=` A).
Proof.
rewrite propeqE; split=> [BA k|]; last first.
  by move=> BA t [k kn] /BA; exact.
by move=> kn t BKt; apply: BA; exists k.
Qed.

(* basic properties of pull-back sigma-algebra *)
Lemma preimage_class_measurable_fun_new d d' (aT : measurableType d) (rT : measurableType d')
    (D : set aT) (f : aT -> rT) :
  preimage_class D f measurable `<=` measurable -> measurable_fun D f.
Proof. by move=> + mD Y mf; apply; exists Y. Qed.

Lemma preimage_class_measurable_fun_new' d d' (aT : measurableType d) (rT : measurableType d')
    (D : set aT) (f : aT -> rT) : measurable D ->
  measurable_fun D f -> preimage_class D f measurable `<=` measurable.
Proof. by move=> mD mf _ [B mB <-]; exact: mf. Qed.

(* generalization of preimage_classes (measure.v) *)
(* sigma-algebra generated by a family of mappings fn indexed by k in I *)
Definition g_sigma_algebra_family I0 (I : set I0) (d : I -> measure_display)
    (Tn : forall i, semiRingOfSetsType (d i)) (T : Type)
    (fn : forall i, T -> Tn i) :=
  <<s \bigcup_(k : I) preimage_class setT (fn k) measurable >>.
Arguments g_sigma_algebra_family {I0} I d Tn {T} fn.

(* the smallest sigma-algebra on X such that all f i are measurable *)
Lemma g_sigma_algebra_family_measurable_fun I0 (I : set I0) (d : I -> measure_display)
    (Tn : forall i, sigmaRingType (d i)) d0 (T : measurableType d0)
    (fn : forall i, T -> Tn i) :
  g_sigma_algebra_family I d Tn fn `<=` measurable ->
  (forall i, measurable_fun setT (fn i)).
Proof.
move=> + i Ii Y mY; apply; apply: sub_sigma_algebra.
by exists i => //; exists Y.
Qed.
...
Lemma g_sigma_algebra_family_measurable_fun' I0 (I : set I0) (d : I -> measure_display)
  ...
  g_sigma_algebra_family I d Tn fn `<=` measurable.
Proof.
move=> h A; apply => /=; split => //; first exact: sigma_algebra_measurable.
by move=> _ [j Ij] [C mC <-]; exact: h.
Qed.

(* cardinal product of a number #|I| of sets *)
Record cart_prod (I0 : Type) (T_ : forall i : I0, Type) : Type := mk_cart_prod {
  coor_map : forall i : I0, T_ i
}.
Arguments cart_prod I0 T_ : clear implicits.

(* alternative of g_sigma_algebra_family *)
Definition product_salgebra (I0 : Type) (I : set I0)
    (d_ : I -> measure_display) (M_ : forall i, semiRingOfSetsType (d_ i))
    : set (set (cart_prod I M_)) :=
  <<s \bigcup_(i : I) [set @coor_map I M_ ^~ i @^-1` A |
                           A in @measurable _ (M_ i) ] >>.
Arguments product_salgebra {I0} I {d_} M_.

Lemma product_salgebraE (I : set nat) (d : I -> measure_display)
    (T : forall n, semiRingOfSetsType (d n)) :
  product_salgebra I T =
  g_sigma_algebra_family I d T (fun k t => coor_map t k)
  :> set (set (cart_prod I T)).
Proof.
rewrite /product_salgebra /g_sigma_algebra_family; congr (<<s _ >>).
...
Definition bigsetX (I : set nat) (T_ : forall i : I, Type)
    (F : forall i, set (T_ i)) : set (cart_prod I T_) :=
  [set x : cart_prod I T_ | forall i : I, F i (coor_map x i)].
Arguments bigsetX I {T_} F.

(* the cardinal product expressed as an intersection *)
Lemma bigsetXE (T : pointedType) (I : set nat) (T_ : forall i : I, Type)
    (F : forall i, set (T_ i)) :
  bigsetX I F = \bigcap_(i : I) (@coor_map I T_ ^~ i @^-1` F i).
Proof.
apply/seteqP; split => [|].
  by move=> s + k Ik/=; exact.
by move=> s/= H i; exact: H.
Qed.

(* the preimage by a coordinate projection expressed as a bigsetX *)
Definition same_or_setT (I : set nat) (i : I) (T_ : forall i : I, Type)
  (A : set (T_ i)) (j : I) : set (T_ j).
move: i A j => [i iI] A [j jI].
case: (PeanoNat.Nat.eq_dec i j) => ij.
  move: jI.
  rewrite <- ij.
  move=> jI.
  rewrite (Prop_irrelevance jI iI).
  exact: A.
exact: setT.
Defined.
Arguments same_or_setT {I} i {T_} A j.

Lemma same_of_setT_eq (I : set nat) (T_ : forall i : I, Type) (i : I)
  (A : set (T_ i)) : same_or_setT i A i = A.
Proof.
rewrite /same_or_setT.
destruct i as [i Hi].
case: PeanoNat.Nat.eq_dec => // ii.
rewrite -Eqdep_dec.eq_rect_eq_dec {ii}; last exact: PeanoNat.Nat.eq_dec.
rewrite /eq_rect_r /= /eq_rect /=.
by suff: Prop_irrelevance Hi Hi = erefl Hi by move=> ->.
Qed.

Lemma same_of_setT_neq (I : set nat) (T_ : forall i : I, Type) (i j : I)
  (A : set (T_ i)) : i != j -> same_or_setT i A j = setT.
Proof.
move=> ij; rewrite /same_or_setT.
destruct i as [i Hi]; destruct j as [j Hj].
case: PeanoNat.Nat.eq_dec => // ij'; subst j.
exfalso.
move/eqP : ij; apply.
by rewrite (Prop_irrelevance Hi Hj).
Qed.

Lemma preimage_bigsetX (T : pointedType) (I : set nat) (T_ : forall i : I, Type)
    i (A : set (T_ i)) :
  (@coor_map I T_ ^~ i) @^-1` A = bigsetX I (fun j => @same_or_setT I i T_ A j).
Proof.
apply/seteqP; split => [s/= Asi j|x/=].
  have [ij|ij] := eqVneq i j.
    by subst j; rewrite same_of_setT_eq.
  by rewrite same_of_setT_neq.
by move=> /(_ i); rewrite same_of_setT_eq.
Qed.

Reserved Notation "( I , d ).-nprod" (format "( I ,  d ).-nprod").
Definition measure_nprod_display (I : set nat) : measure_display -> measure_display.
Proof. exact. Qed.
Notation "( I , d ).-nprod" := (measure_nprod_display I d) : measure_display_scope.

(* measurableType for the product of a number #|I| of sets *)
Section nproduct_salgebra_instance.
Context (I : set nat) {d : I -> measure_display}
  (T : forall i : I, semiRingOfSetsType (d i)).

Notation C := (product_salgebra I T).

Let nprod_salgebra_set0 : C (set0 : set (cart_prod I T)).
Proof. exact: sigma_algebra0. Qed.

Let nprod_salgebra_setC A : C A -> C (~` A).
Proof.
rewrite /C /g_sigma_algebra_family.
set tmp := bigcup _ _.
(* broken here because of pointed types *)
exact: sigma_algebraC.
Qed.

Let nprod_salgebra_bigcup (F : _^nat) : (forall i, C (F i)) ->
  C (\bigcup_i (F i)).
Proof. exact: sigma_algebra_bigcup. Qed.

HB.instance Definition nprod_salgebra_mixin :=
  @isMeasurable.Build ((I, d).-nprod)%mdisp (seq T) C
    (nprod_salgebra_set0) (nprod_salgebra_setC) (nprod_salgebra_bigcup).

End nproduct_salgebra_instance.
Notation "( I , p ).-nprod.-measurable" :=
  (((I, p).-nprod).-measurable : set (set (seq _))) :
    classical_set_scope.

Lemma bigsetX_measurable {d : measure_display} (T : semiRingOfSetsType d)
  (I : set nat) (F : nat -> set T) : I !=set0 ->
  (forall n, I n -> d.-measurable (F n)) ->
  (I,d).-nprod.-measurable (bigsetX I F).
Proof.
move=> I0 mF; apply: bigcap_measurable => //= k Ik.
apply: sub_sigma_algebra; exists k => //; exists (F k) => //.
exact: mF.
Qed.

Section product_salgebra_algebraOfSetsType.
Context {d : measure_display} (T : algebraOfSetsType d) (I : set nat).
Let Mn := [set B | exists2 A : nat -> set T, B = bigsetX I A &
                   forall i, I i -> A i \in @measurable _ T].

(* the product sigma-algebra is the algebra generated by the cartesian product *)
Lemma measurable_prodn_measurableType : I !=set0 ->
  product_salgebra I T = <<s Mn >>.
Proof.
move=> I0; rewrite eqEsubset; split.
  apply: smallest_sub => //.
  rewrite subbigcupset => k nk _/= [A mA <-].
  apply: sub_sigma_algebra.
  exists (fun j => if j == k then A else [set: T]).
    by rewrite (@preimage_bigsetX _ I).
  by move=> i; rewrite inE; case: ifPn.
apply: smallest_sub; first exact: smallest_sigma_algebra.
move=> _ [F -> mF].
apply: bigsetX_measurable => // k Ink.
exact/set_mem/mF.
Qed.

End product_salgebra_algebraOfSetsType.

Section test_expectation.
Local Open Scope ereal_scope.
Context d (T : measurableType d) (R : realType) (P : probability T R).

Definition nproduct (I : set nat) :
  probability ([the measurableType (I, d).-nprod of seq T]) R.
Admitted.

Lemma expectation_prod_independent_RVs n (X : {RV (nproduct `I_n) >-> R}^nat) :
  independent_RVs (nproduct `I_n) `I_n X ->
  'E_(nproduct `I_n)[\prod_(i < n) (X i)] =
  \prod_(i < n) 'E_(nproduct `I_n)[X i].
Proof.
Admitted.

End test_expectation.

Section joint_distribution_function.
Context {R : realType} d (T : measurableType d).
Variable P : probability T R.
Local Open Scope ereal_scope.

Definition joint_distribution_function
    (X : nat -> {mfun T >-> R}) (J : {fset nat}) :=
  fun x : seq R => P (\big[setI/setT]_(j <- J) X j @^-1` `]-oo, nth point x j]%classic).

Lemma thm221 (X : nat -> {mfun T >-> R}) (I : set nat) :
  independent_RVs P I X <-> forall J : {fset nat}, ([set` J] `<=` I) ->
    forall x : seq R,
      joint_distribution_function X J x =
      \prod_(j <- J) joint_distribution_function X [fset j]%fset [:: (nth point x j)].
Proof.
split.
  move=> H J JI s.
  rewrite /joint_distribution_function/=.
  under [in RHS]eq_bigr do rewrite big_seq_fset1.
  case: H => /= _ /(_ _ JI).
  move=> /(_ (fun j => X j @^-1` `]-oo, (nth point s j)])) => ->//.
    admit.
  move=> k kJ.
  rewrite inE/=.
  rewrite /g_sigma_algebra_mapping.
  rewrite /preimage_class/=.
  exists (`]-oo, (nth point s k)]%classic) => //.
  by rewrite setTI.
move=> H.
apply: independent_generators => //=.
- move=> i Ii A B mA mB.
  by apply: measurableI.
- by move=> i Ii A -[[x1 x2] _ <-{A}/=].
- split => [i Ii s|] /=.
  + move=> [A mA] <-.
    rewrite setTI.
