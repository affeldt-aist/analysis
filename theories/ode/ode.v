...
(*   delta_max == min (b - a, r / (k * r + hmax), rho / k)                    *)
...
Theorem cauchy_lipschitz_local : exists f delta,
  delta > 0 /\
  is_sol_on phi a (a + delta) u0 f /\
  {in `[a, a + delta], forall t, closed_ball u0 r%:num (f t)} /\
  {within `[a, a + delta], continuous f}.
Proof.
exists (repr (picard_fix ab k0 lip2 cont1 rho1)), delta_max.
...
Context {R : realType} {n : nat}.
Notation U := 'rV[R]_n.
Variables (phi : R -> U -> U) (a b : R) (k : R) (u0 : U) (r : {posnum R}) (f f' : R -> U).
Hypothesis ab : a < b.
Hypothesis k0 : 0 < k.
Let B := closed_ball u0 r%:num.
Hypothesis lip2 : {in `[a, b]%R, forall x : R, k.-lipschitz_B (phi x)}.
...
move => h1 h2 t tab.
have /= := cauchy_lipschitz_unique lip2 cont1 rho1.
Admitted.
...
have [f [d [d0 [solf [cball cf]]]]] :=
  cauchy_lipschitz_local aa1 k0 (@phi_lip2 a (a + 1)) (@phi_cont1 a (a + 1)).
by exists f, d.
Qed.
