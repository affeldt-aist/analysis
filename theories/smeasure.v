...
Context d (T : measurableType d) (R : realType).
Variable mu : {smeasure set T -> \bar R}.

Variable D : set T.

Let elt_prop (x : set T * \bar R * set T) :=
  [/\ measurable x.1.1,
     0 <= mu x.1.1,
     0 <= x.1.2,
     x.1.1 `<=` D &
...
Let elt_A_ge0 x : 0 <= mu (A_ x).
Proof. by move: x => [[[? ?] ?]] []. Qed.

Let elt_d_ge0 x : 0 <= d_ x.
Proof. by move: x => [[[? ?] ?]] []. Qed.

Let elt_mine x : mu (A_ x) >= mine (d_ x * 2^-1%:E) 1.
Proof. by move: x => [[[? ?] ?]] []. Qed.
...
Let set_mE A :=
  [set mE | exists E, [/\ mE = mu E, measurable E & E `<=` D `\` A] ].
...
Lemma hahn_decomp_lemma : measurable D -> mu D <= 0 ->
...
Let elt_prop (A : set T * \bar R * set T) :=
  [/\ measurable A.1.1,
     nu A.1.1 <= 0,
     A.1.2 <= 0,
...
have [A0 [mA0 muA0 _ negA0 A0d0]] := next_elt ss0.
have [v [v0 Pv]] : {v : nat -> elt_type |
    v 0%N = exist _ (A0, ss set0, A0) (And5 mA0 muA0 ss0 negA0 A0d0) /\
    forall n, seq_inf (v n) (v n.+1)}.
  apply dependent_choice_Type => -[[[A' d'] U'] [/= mAn nuAn_le0 negAn]].
  pose sn1 := ss U'.
  have sn1_le0 : sn1 <= 0.
    by apply: ereal_inf_lb => /=; exists set0; split => //; rewrite smeasure0.
  have [A1 [mA1 nuA1_le0 ? negA1 A1d1] ] := next_elt sn1_le0.
  by exists (exist _ (A1, sn1, U' `|` A1) (And5 mA1 nuA1_le0 sn1_le0 negA1 A1d1)).
